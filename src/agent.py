...
from openrag_agent_system_prompt import OPENRAG_AGENT_SYSTEM_PROMPT
...
def get_conversation_thread(user_id: str, previous_response_id: str = None):
    ...
    new_conversation = {
        "messages": [
            {
                "role": "system",
                "content": OPENRAG_AGENT_SYSTEM_PROMPT,
            }
        ],
...
async def async_langflow_chat(
    ...
    if not (prompt or "").strip():
        log_event(
            logger,
            "langflow_invoke_rejected_empty_prompt",
            level="error",
            flow_id=flow_id,
            store_conversation=store_conversation,
        )
        raise ValueError("Prompt is required for Langflow chat")
...
async def async_langflow_chat_stream(
    ...
    if not (prompt or "").strip():
        log_event(
            logger,
            "langflow_stream_rejected_empty_prompt",
            level="error",
            flow_id=flow_id,
        )
        raise ValueError("Prompt is required for Langflow chat")
